...
function FieldError({
  ...
  const content = useMemo(() => {
    // Explicit children take full control
    if (children) {
      return children
    }
    ...
    // Array of errors (e.g. react-hook-form field error)
    if (errors?.length) {
      const uniqueErrors = [
      ...
      return (
        <ul className="ml-4 flex list-disc flex-col gap-1">
          {uniqueErrors.map(
            (e, index) =>
              e?.message && <li key={index}>{e.message}</li>
          )}
        </ul>
      )
    }
  ...
  return (
    <div
      ...
      className={cn("inline-flex items-center gap-1 text-sm font-normal text-danger", className)}
      {...props}
    >
      {content}
    </div>
