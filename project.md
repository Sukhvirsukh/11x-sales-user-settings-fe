...
- **Surfaces**: `--background` (#FFF), `--popover` (#FFF), `--sidebar-bg` (#ffffff), `--surface-raised` (#ffffff), `--surface-subtle` (#F7F8FB), `--muted` (#F1F1F1). The three primary container primitives have dedicated layers: `--preview-section-background` (#FFF), `--app-section-background` (the pale blue section tint), and `--app-card-background` (#F8FAFD); their dark values progressively lighten from preview → section → nested card so hierarchy remains visible.
